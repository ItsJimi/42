...
// View all img
router.get('/img/view/:username/:size/:nbr', function (req, res) {
	db.get("profiles", function(json) {
		if (!json[0]) {
			res.sendStatus(404);
			return (false);
		}
		var nbr = parseInt(req.params.nbr);
		if (!nbr || nbr < 0 || nbr > 4)
			nbr = 0;
		if (json[0].img && nbr < json[0].img.length)
			var img = json[0].img[nbr];
		else
...
// Delete your img
router.post('/img/del', function (req, res) {
	var sess = req.session;
