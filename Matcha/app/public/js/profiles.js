...
function viewProfile(callback, username) {
	...
	$('#profiles_time_s').html("...");
	$('#profiles_score_s').html("0");
	...
	$.get("/api/profiles/view/" + username, {}).done(function(res) {
		...
		if (res[0].time)
			$('#profiles_time_s').html(moment(moment(res[0].time).format('YYYYMMDDHHmmss'), "YYYYMMDDHHmmss").fromNow());
		else if (res[0].time == false)
			$('#profiles_time_s').html("ONLINE");
		if (res[0].score)
			$('#profiles_score_s').html(res[0].score);
		if (res[0].location)
